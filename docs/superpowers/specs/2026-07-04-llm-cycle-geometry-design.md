...
- **Phase 6 — Intrinsic ring topology in activation space.** Beyond Part 1's top-2 circle-fit: test whether the intrinsic manifold is a ring (S¹) even if curved/non-axis-aligned. **PCA embedding** (linear first pass) + **graph Laplacian / Laplacian eigenmaps** (k-NN graph over state activations; lowest non-trivial eigenvectors form a `cos`/`sin` pair iff circular) + (later) persistent homology. Pairs with densification (topology needs many points). Input: a forthcoming paper summary on ring topology in neuronal data.
