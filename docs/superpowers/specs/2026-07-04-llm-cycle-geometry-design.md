...
- **Error bars — from the spread across runs (no bootstrap in v1).** With only ~16 templates a formal bootstrap CI would be coarse (its precision is bounded by the ~16 *unique* runs, not by the iteration count) and add machinery without real value. v1 instead takes error bars **directly from the runs**: project **each target run separately** onto the source PCs to get one `AUC_cross` per run, and take one `AUC_within` per LOO fold, then report **mean ± SEM across runs** (or the run-wise min–max range). This is transparent and uses the actual `T` runs as the unit. *(Deferred: a proper bootstrap CI once we have many more templates and/or multiple models to support one.)*
...
- **Phase 6 — Ring structure in *activation* space (distinct from Part 1's *representation* space).** The distinction from Part 1 is **which side of the SVD the analysis is built on** — *not* whether we average. Part 1 characterizes the arrangement of the **state representations**, read off the **`V` (state) side** (the top state-modes give one point per state, tracing the ring). Phase 6 instead asks whether a ring appears in the **activation space**, with the analysis built from the **`U` (activation / hidden-unit) side** rather than `V`. Averaging over templates is fine, exactly as in Part 1.
  - This mirrors how the neuronal-data papers characterize ring/torus structure in population activity (head-direction ring, grid-cell torus). Candidate tools — **PCA**, **graph-Laplacian / Laplacian eigenmaps**, later **persistent homology** — but the **exact construction is deferred**: we'll design the Phase-6 analysis in a later pass, using the forthcoming neuronal-data paper summary as input. Whether densification (more states/templates) is needed will be decided then.
